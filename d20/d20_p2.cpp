...
For part 2, we make it such that the moves can be at most 20
Takes a bit but its still fast enough to get the correct answer
...
int main()
{
    ...
    for(int row = -20; row <= 20; row++)
    {
        ...
        {
            if (abs(row) + abs(col) > 20) continue;
            vector<int> temp = {row,col,abs(row) + abs(col)};
    ...
    for(int row = 0; row < total_rows; row++)
    {
        for(int col = 0; col < total_cols; col++)
        {
            ...
            for(auto &move : moves)
            {
                ...
                if (r >= 0 && r < total_rows && c >= 0 && c < total_cols && dist[new_idx] - dist[curr_idx] - w >= 100 && matrix[r][c] != '#')
                {
                    cout << "start " << "r: " << row << " c:" << col << " end r:" << r << " c:" << c;
                    cout << " saving: " << dist[new_idx] - dist[curr_idx] - w << endl;     
                    ans++;
                }
